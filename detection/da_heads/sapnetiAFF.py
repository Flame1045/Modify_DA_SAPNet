...
class SAPNetiAFF(nn.Module):
    ...
    def __init__(self, *, num_anchors, in_channels, embedding_kernel_size=3,
        ...
        self.mscam = MSCAM(channels=in_channels+num_anchors, r=4)
        self.iaff = iAFF(channels=in_channels+num_anchors, r=4)  ##############modify#################
    ...
    def forward(self, feature, rpn_logits, input_domain):
        ...
        semantic_map = self.iaff(semantic_map, semantic_map) # iAFF
        ...
        fuse = sum(pyramid_features)  # [N, 256, 1, 1] ##############modify#################
        # print(len(pyramid_features))
        # fuse = pyramid_features[0]
        # for i, k in enumerate(self.window_sizes):
        #     if i < 12:
        #         fuse  = self.iaff(fuse, pyramid_features[i+1])##############modify#################
        ...
        #print("A",len(pyramid_features))
        

        final_features = sum(pyramid_features)  ##############modify#################
        #print("B",final_features.shape)

        # final_features = pyramid_features[0]
        # for i, k in enumerate(self.window_sizes):
        #     if i < 12:
        #         final_features  = self.iaff(final_features, pyramid_features[i+1])##############modify#################

        del pyramid_features, w, split, merge, fuse, feature, rpn_logits
        ##############modify#################
        # final_features = fuse.view(N, -1) # semantic vector
        final_features = final_features.view(N, -1) # semantic vector
        #print("C",final_features.shape)
