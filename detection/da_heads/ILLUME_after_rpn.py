...
class SAPNet_ILLUME_a(nn.Module):
    ...
    def forward(self, feature, rpn_logits, input_domain):
        ...
        # print("ILLUME-A")
        _, ch, h, w = semantic_map.size()
        # Theta path
        theta = self.snconv1x1_theta(semantic_map)
        theta = theta.view(-1, ch//8, h*w)

        # Phi path
        phi = self.snconv1x1_phi(semantic_map)
        ...
        # g path
        g = self.snconv1x1_g(semantic_map)
        g = self.maxpool(g)
        ...
        #Out
        att = semantic_map + self.sigma*attn_g
