...
class SAPNetiAFFadapter(nn.Module):
    ...
    def __init__(self, *, num_anchors, in_channels, embedding_kernel_size=3,
        ...
        self.inter_channel = num_anchors // R
        self.alpha = Alpha
        ...
        self.adapter = nn.Sequential(
            ...
            nn.Conv2d(num_anchors, self.inter_channel, kernel_size=embedding_kernel_size, padding=1, 
                      groups=num_anchors),
            # Non-linear Activation
            nn.ReLU(True),
            # Pointwise Convolution
            nn.Conv2d(self.inter_channel, in_channels, kernel_size=1, groups=1),
            )
        
        if self.alpha == 'ones':
            self.scalar= nn.Parameter(torch.ones(in_channels))
        else:
            assert False , "ALPHA should be ones"

        #self.mscam = MSCAM(channels=in_channels+num_anchors, r=1)
        self.iaff = iAFF(channels=in_channels, r=1)
    ...
    def forward(self, feature, rpn_logits, input_domain):
        ...
        rpn_feature = self.adapter(rpn_feature)
